...
to_cost = [0]*(num_city+1)

def bfs(start):
    todo = []
    for cost,next in rute[start]:
        to_cost[next] = cost
        heapq.heappush(todo,(cost,next))
    while todo:
        total_cost,now = heapq.heappop(todo)
        if to_cost[now] < total_cost:
            continue
        for cost,next in rute[now]:
            if not to_cost[next] or to_cost[next] > cost+total_cost:
