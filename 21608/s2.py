...
# 좋아하는 애가 없을 때
def chose_seat(sat_value):
    max_value = -1
    for c in range(1,num_col+1):
        for r in range(1, num_col+1):
            value = 0
            if seat[c][r] or sat_priority[c][r]<sat_value:
                continue
            for i in range(4):
                new_col = m_c[i] + c
                new_row = m_r[i] + r
...
def set_sat(pos, student_id):
    ...
    student_to_position[student_id] = pos
    for i in range(4):
        new_col = m_c[i] + pos[0]
        new_row = m_r[i] + pos[1]
...
for _ in range((num_col**2)-1):
    ...
    for j in range(1,5):
        ...
        if start_pos == 0:
            continue
        value = like(start_pos)
        if value > max_value:
            max_value = value
            continue
    student_to_sat[student_id] = max_value
    ans_pos = chose_seat(max_value)
    set_sat(ans_pos, student_id)

ans = 0
for i in range(1, num_col**2+1):
    if student_to_sat[i]:
