...
def interval(start, end, index, left, right, target):
    # 범위 밖에 있는 경우
    ...
    if left <= start and right >= end:
        return tree[index]
    mid = (start + end) // 2
    a = interval(start, mid, index * 2, left, right,target)
    if a <target:
        return 0
    b = interval(mid + 1, end, index * 2 + 1, left, right,target)
    if b <target:
        return 0
...
def init(start, end, index):
    ...
    tree[index] = left if left<right else right
    return tree[index]
