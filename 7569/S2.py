...
visited = [[[maxsize]*(num_row) for _ in range(num_col)] for i in range(num_flour)]
    ...
    while que:
        ...
        for i in range(6):
            ...
            if next_r <0 or next_r > num_row-1:
                continue
            if visited[next_f][next_c][next_r] >dis+1 and graph[next_f][next_c][next_r]!=-1:
                visited[next_f][next_c][next_r] = dis+1
    ...
    for f in range(num_flour):
        for c in range(num_col):
            for r in range(num_row):
                if graph[f][c][r]==0 and visited[f][c][r] > max_dis:
                    return -1
    return max_dis
a = bfs()
print(start(a))